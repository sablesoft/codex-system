...
module TerminalImageLink
  ...
  KITTY_BLOCK_START = "# >>> codex-system terminal-link image open_actions >>>"
  KITTY_BLOCK_END = "# <<< codex-system terminal-link image open_actions <<<"

  def preview_lines(path:, kitty_label: "[Show image]", fallback_label: nil)
    file_link_lines(path: path, kind: "image", kitty_label: kitty_label, fallback_label: fallback_label)
  end

  def file_link_lines(path:, kind: "auto", kitty_label: "[Show image]", fallback_label: nil)
